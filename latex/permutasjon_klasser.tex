...
\begin{algorithm}
  ...
  \begin{algorithmic}[0]
    ...
    \Procedure {permute}{$Ontology:ontology, List:permutables, Class:currentClass, Stack:treeStack$}
    %	\State flush reasoner
    $treeClass \leftarrow currentClass$

    \If {$currentClass$ is a leaf class}
    ...
    %\State $Class:superClass \leftarrow$ root class in the ontology
    ...
    \ForAll {$axiomSet \in axiomSets$}
        \State $Set:newAxioms$
        \State
        \ForAll {$a \in axiomSet$}
            \If {$a \notin ontology$}
        ...
        \ForAll {$c_d \in directSubclasses$}
